...
export default function LogicGate() {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
      <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-1">Logic gate simulator</h2>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Toggle inputs and see the output in real time</p>
        ...
        {Object.keys(gates).map(g => (
          <button key={g} onClick={() => setGate(g)}
            className={gate === g
              ...
              : 'px-3 py-1 rounded-lg border border-gray-200 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}>
            {g}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-3 mb-4 justify-center">
        <button onClick={() => setA(!a)}
          className={`w-24 py-2 rounded-lg text-sm font-medium border transition-all ${a ? 'bg-green-500 text-white border-green-500' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700'}`}>
          A = {a ? '1' : '0'}
        </button>
        {needsTwo && (
          <button onClick={() => setB(!b)}
            className={`w-24 py-2 rounded-lg text-sm font-medium border transition-all ${b ? 'bg-green-500 text-white border-green-500' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-700'}`}>
            B = {b ? '1' : '0'}
          </button>
        )}
      </div>
      <div className={`text-center py-3 rounded-xl text-sm font-medium ${output ? 'bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900 text-red-600 dark:text-red-300'}`}>
        Output = {output ? '1 (HIGH)' : '0 (LOW)'}
      </div>
      <p className="text-xs text-gray-400 dark:text-gray-500 text-center mt-3">{gateDescriptions[gate]}</p>
    </div>
  )
}