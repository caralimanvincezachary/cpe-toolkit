...
export default function CircuitNotes() {
  ...
  useEffect(() => {
    localStorage.setItem('circuit-notes', JSON.stringify(notes))
  }, [notes])
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      <h2 className="text-lg font-medium text-gray-900 mb-1">Circuit notes</h2>
      <p className="text-sm text-gray-500 mb-4">Save notes about circuits, formulas, and components</p>

      <div className="bg-gray-50 rounded-xl p-4 mb-5">
        <input type="text" value={title} onChange={e => setTitle(e.target.value)}
          placeholder="Note title..."
          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-2 focus:outline-none focus:border-blue-400 bg-white" />
        <textarea value={body} onChange={e => setBody(e.target.value)}
          placeholder="Write your note... formulas, pin configs, observations"
          rows={4}
          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-2 focus:outline-none focus:border-blue-400 bg-white resize-none" />
        <div className="flex gap-2 flex-wrap mb-2">
          {categories.map(c => (
            <button key={c} onClick={() => setCat(c)}
              className={cat === c
                ? 'px-3 py-1 rounded-lg bg-blue-600 text-white text-xs font-medium'
                : 'px-3 py-1 rounded-lg border border-gray-200 text-xs text-gray-500 hover:bg-white'}>
              {c}
            </button>
          ))}
        </div>
        ...
        {editing !== null && (
          <button onClick={() => { setEditing(null); setTitle(''); setBody(''); setCat('General') }}
            className="w-full py-2 rounded-lg border border-gray-200 text-sm text-gray-500 mt-2">
            Cancel edit
          </button>
        )}
      </div>
      ...
      <div className="flex gap-2 flex-wrap mb-4">
        {['All', ...categories].map(c => (
          <button key={c} onClick={() => setFilter(c)}
            className={filter === c
              ? 'px-3 py-1 rounded-lg bg-gray-900 text-white text-xs'
              : 'px-3 py-1 rounded-lg border border-gray-200 text-xs text-gray-500 hover:bg-gray-50'}>
            {c}
          </button>
        ))}
      </div>
        ...
        {filtered.map((n, i) => (
          <div key={i} className="border border-gray-200 rounded-xl p-4">
            <div className="flex justify-between items-start mb-1">
              ...
              <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-lg ml-2 flex-shrink-0">{n.cat}</span>
            </div>
            <p className="text-sm text-gray-600 whitespace-pre-wrap mb-2">{n.body}</p>
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-400">{n.date}</span>
              <div className="flex gap-2">
                <button onClick={() => edit(notes.indexOf(n))}
                  className="text-xs text-blue-500 hover:text-blue-700">Edit</button>
                <button onClick={() => del(notes.indexOf(n))}
