...
export default function NumberQuiz() {
  ...
  function restart() {
    setQs(shuffle(questions).slice(0, 5))
    setCurrent(0)
    setInput('')
    setStatus(null)
    setScore(0)
  ...
  if (done) return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      <h2 className="text-lg font-medium text-gray-900 mb-1">Quiz complete!</h2>
      ...
      <div className="space-y-2 mb-5">
        {results.map((r, i) => (
          <div key={i} className={`p-3 rounded-xl text-sm ${r.ok ? 'bg-green-50' : 'bg-red-50'}`}>
            <p className="font-medium text-gray-700">{r.q}</p>
            {!r.ok && <p className="text-red-500 text-xs mt-1">Your answer: {r.your} — Correct: {r.correct}</p>}
            {r.ok && <p className="text-green-500 text-xs mt-1">Correct! ✓</p>}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-medium text-gray-900">Number system quiz</h2>
        <span className="text-sm text-gray-400">{current + 1} / {qs.length}</span>
      </div>
      ...
      <div className="bg-gray-50 rounded-xl p-4 mb-4">
        <p className="text-base font-medium text-gray-800">{qs[current].q}</p>
      </div>
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-3 focus:outline-none focus:border-blue-400 disabled:bg-gray-50"
      />

      {status === 'correct' && (
        <div className="bg-green-50 text-green-700 rounded-xl px-4 py-3 text-sm font-medium mb-3">
          ✓ Correct!
        </div>
      )}
      {status === 'wrong' && (
        <div className="bg-red-50 text-red-600 rounded-xl px-4 py-3 text-sm mb-3">
          ✗ Wrong — the answer is <span className="font-mono font-bold">{qs[current].a}</span>
        </div>
      )}
      {showHint && !status && (
        <div className="bg-yellow-50 text-yellow-700 rounded-xl px-4 py-3 text-sm mb-3">
          💡 {qs[current].hint}
        </div>
      )}
      ...
      <p className="text-xs text-gray-400 text-center mt-3">Score: {score} correct so far</p>
    </div>
  )
}